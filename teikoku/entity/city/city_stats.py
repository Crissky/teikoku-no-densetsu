...
from teikoku.data.stats import CITY_LEVEL_GROWUP
from teikoku.entity.unit.stats_modifier import StatModifier
from teikoku.enum.stats import CityStatsEnum


@dataclass
class CityStats(MongoBase):
    level: int
    damaged: int
    base_hp: int
    base_attack: int
    ...
    def _get_modified_stat(
        ...
        level_growup = self.level * CITY_LEVEL_GROWUP[attr]
    ...
    @property
    def hp(self) -> int:
        return self._get_modified_stat(CityStatsEnum.HP)

    @property
    def attack(self) -> int:
        return self._get_modified_stat(CityStatsEnum.ATTACK)

    @property
    def defense(self) -> int:
        return self._get_modified_stat(CityStatsEnum.DEFENSE)
    ...
    def telegram_text(self) -> str:
        return (
            ...
            f"{self.attack_emoji}: {self.attack}({self.base_attack})\n"
